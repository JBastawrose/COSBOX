...
typedef enum
{
    ...
    RLState_NoVideoAuthorization = -6,
} RedLaserStatus;
...
/*******************************************************************************
    RL_RequestVideoAuthorization

    This function returns immediately and fires a completion handler block with the state of the user permissions.
    This completion handler fires on an arbitrary queue that is not necessarily the main queue.
    The SDK will call your completion handler with RL_VideoAuthorizationStatusAuthorized to indicate that the user has granted permission to access the video camera.
*/

typedef NS_ENUM(NSInteger, RL_VideoAuthorizationStatus) {
    RL_VideoAuthorizationStatusNotDetermined = 0,
    RL_VideoAuthorizationStatusRestricted,
    RL_VideoAuthorizationStatusDenied,
    RL_VideoAuthorizationStatusAuthorized
};

typedef void (^RL_RequestVideoAuthorizationCompletionHandler)(RL_VideoAuthorizationStatus);

void RL_RequestVideoAuthorization(RL_RequestVideoAuthorizationCompletionHandler);
